...
export default function PlayerFocusedEditor({
  ...
  const handlePositionDragEnd = (slideIndex: number, newX: number, newY: number) => {
    if (!canEdit) return;
      ...
      {canEdit && (
        <div className="bg-blue-50 border-b border-blue-200 px-4 py-2">
          <p className="text-sm text-blue-800">
            <strong>Tap canvas</strong> to place {selectedPlayer?.label}'s next position •
            ...
            {selectedPlayerPositions.map((pos, index) => (
              <Group key={`${pos.id}-${pos.slideIndex}`}>
                ...
                <Circle
                  ...
                  draggable={canEdit}
                  onDragEnd={(e) => {
                    handlePositionDragEnd(pos.slideIndex, e.target.x(), e.target.y());
                  }}
                  shadowColor="black"
                  shadowBlur={5}
                  shadowOpacity={0.3}
